    ...
    datagen(datalist){
                ...
                if(this.country_list.find(obj=>obj===row.country)){
                    ...
                    if(!pushObj){
                        if(row.date!==this.dateList[0]){
                            ...
                            if(i==='0') crr[row.country]={install: [0], spend: [1]};
                            ...
                            brr[parseInt(1)+parseInt(i)].datasets.push({
                                ...
                                borderColor: this.color[brr[parseInt(1)+parseInt(i)].datasets.length-1],
                                tension: 0.5,
                                borderWidth:1,
                            })
                            if(i==='0') crr[row.country]={install: [row.install], spend: [row.spend]};
                        }
                    }
                    else {
        ...
        for(let i=0; i<this.country_list.length; i++){
            brr[0].datasets.push({
                ...
                data: [],
                borderColor: this.color[brr[0].datasets.length-1],
                tension: 0.5,
                borderWidth:1,
            })
